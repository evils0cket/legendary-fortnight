...
def query_api(srcip, apikey):
    ...
    headers = {"Accept-Encoding": "gzip, deflate",'Accept': 'application/json',"Key": apikey}
...
def request_abuseipdb_info(alert, apikey):
    ...
    if not "aws" in alert["data"]:
        return(0)

    # Request info using AbuseIPDB API
    data = query_api(alert["data"]["aws"]["srcaddr"], apikey)
...
#   alert_output["abuseipdb"]["source"]["full_log"] = alert["full_log"]
    alert_output["abuseipdb"]["source"]["srcip"] = alert["data"]["aws"]["srcaddr"]
    srcip = alert["data"]["aws"]["srcaddr"]
